...
export const AcademicSessionSetupPage: React.FC = () => {
  ...
  const [editingCohort, setEditingCohort] = useState({ name: '', grade: '', section: '', academicYear: '', description: '' });
  ...
  useEffect(() => {
    Promise.all([getAcademicCohorts(), getAcademicRubrics()])
      .then(([cohortResponse, rubricResponse]) => {
        ...
        setRubricIds(rubricResponse.data[0] ? [rubricResponse.data[0].id] : []);
      })
  ...
  const handleSubmit = async (event: React.FormEvent, shouldSchedule: boolean) => {
    ...
    setError('');
    if (!cohortId || !rubricIds.length) return setError('Select a cohort and at least one rubric first.');
  ...
  const startEditingCohort = (cohort: AcademicCohort) => {
    setEditingCohortId(cohort.id);
    setEditingCohort({ name: cohort.name, grade: cohort.grade, section: cohort.section, academicYear: cohort.academicYear, description: cohort.description || '' });
  };
  ...
  const saveCohort = async (event: React.FormEvent) => {
    ...
    try {
      const response = await updateAcademicCohort(editingCohortId, editingCohort);
      setCohorts((current) => current.map((cohort) => cohort.id === response.data.id ? response.data : cohort));
      setEditingCohortId('');
    } catch (requestError: any) { setError(requestError.response?.data?.detail || 'Unable to update cohort.'); }
    finally { setSetupSaving(false); }
  };
  ...
  return (
    <div className="page-shell mx-auto max-w-5xl space-y-6">
      ...
      {!!cohorts.length && <section className="ui-card space-y-4 p-6"><div><h2 className="text-base font-semibold text-slate-800">Saved cohorts</h2><p className="mt-1 text-sm text-slate-500">Edit a cohort whenever its class details change.</p></div><div className="grid gap-3 md:grid-cols-2">{cohorts.map((cohort) => editingCohortId === cohort.id ? <form key={cohort.id} onSubmit={saveCohort} className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-4"><div className="grid gap-2 sm:grid-cols-2"><input required value={editingCohort.name} onChange={(event) => setEditingCohort({ ...editingCohort, name: event.target.value })} className="ui-input" placeholder="Cohort name" /><input required value={editingCohort.grade} onChange={(event) => setEditingCohort({ ...editingCohort, grade: event.target.value })} className="ui-input" placeholder="Grade" /><input required value={editingCohort.section} onChange={(event) => setEditingCohort({ ...editingCohort, section: event.target.value })} className="ui-input" placeholder="Section" /><input required value={editingCohort.academicYear} onChange={(event) => setEditingCohort({ ...editingCohort, academicYear: event.target.value })} className="ui-input" placeholder="Academic year" /></div><div className="mt-3 flex gap-2"><button disabled={setupSaving} className="academic-primary-button"><Save size={15} /> Save</button><button type="button" onClick={() => setEditingCohortId('')} className="rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-600">Cancel</button></div></form> : <div key={cohort.id} className="flex items-center justify-between rounded-xl border border-slate-200 p-4"><div><strong className="block text-sm text-slate-700">{cohort.name}</strong><span className="text-xs text-slate-500">{cohort.grade} · Section {cohort.section} · {cohort.academicYear}</span></div><button type="button" onClick={() => startEditingCohort(cohort)} className="text-xs font-semibold text-indigo-600 hover:text-indigo-800">Edit</button></div>)}</div></section>}
      {!cohorts.length || !rubrics.length ? (
        ...
        <>
          <section className="grid gap-4 md:grid-cols-2">
            <form onSubmit={createCohort} className="ui-card space-y-3 p-5">
              <div><h2 className="text-sm font-semibold text-slate-800">Add another cohort</h2><p className="mt-1 text-xs text-slate-500">Create cohorts for other classes or sections.</p></div>
              <div className="grid grid-cols-2 gap-2"><input required placeholder="Name" value={cohortForm.name} onChange={(event) => setCohortForm({ ...cohortForm, name: event.target.value })} className="ui-input" /><input required placeholder="Grade" value={cohortForm.grade} onChange={(event) => setCohortForm({ ...cohortForm, grade: event.target.value })} className="ui-input" /><input required placeholder="Section" value={cohortForm.section} onChange={(event) => setCohortForm({ ...cohortForm, section: event.target.value })} className="ui-input" /><input required placeholder="Academic year" value={cohortForm.academicYear} onChange={(event) => setCohortForm({ ...cohortForm, academicYear: event.target.value })} className="ui-input" /></div>
              <button disabled={setupSaving} className="academic-outline-button"><Plus size={15} /> Add cohort</button>
            </form>
            <form onSubmit={createRubric} className="ui-card space-y-3 p-5">
              <div><h2 className="text-sm font-semibold text-slate-800">Add another rubric</h2><p className="mt-1 text-xs text-slate-500">Create reusable judgement criteria and select multiple below.</p></div>
              <input required placeholder="Rubric name" value={rubricName} onChange={(event) => setRubricName(event.target.value)} className="ui-input" />
              <button disabled={setupSaving} className="academic-outline-button"><Plus size={15} /> Add rubric</button>
            </form>
          </section>
          ...
          <section className="ui-card space-y-5 p-6">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="space-y-1.5"><span className="ui-label">Rubrics</span><div className="space-y-2 rounded-xl border border-slate-200 p-3">{rubrics.map((rubric) => <label key={rubric.id} className="flex cursor-pointer items-center gap-3 text-sm text-slate-700"><input type="checkbox" checked={rubricIds.includes(rubric.id)} onChange={() => setRubricIds((current) => current.includes(rubric.id) ? current.filter((id) => id !== rubric.id) : [...current, rubric.id])} className="h-4 w-4 accent-indigo-600" /><span>{rubric.name} · {rubric.dimensions.length} dimensions</span></label>)}</div></div>
          ...
          <section className="ui-card space-y-5 p-6">
            ...
            <div className="grid gap-3 rounded-xl border border-dashed border-indigo-200 bg-indigo-50/50 p-4 md:grid-cols-[.7fr_1fr_1fr_auto]"><input required placeholder="Student ID" value={studentForm.studentId} onChange={(event) => setStudentForm({ ...studentForm, studentId: event.target.value })} className="ui-input" /><input required placeholder="Student name" value={studentForm.name} onChange={(event) => setStudentForm({ ...studentForm, name: event.target.value })} className="ui-input" /><input type="email" placeholder="Email (optional)" value={studentForm.email} onChange={(event) => setStudentForm({ ...studentForm, email: event.target.value })} className="ui-input" /><button type="button" disabled={setupSaving || !studentForm.studentId.trim() || !studentForm.name.trim()} onClick={addStudent} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white disabled:opacity-50"><Plus size={15} /> Add student</button></div>
          </section>

          <section className="ui-card flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Selected rubrics</p><p className="mt-1 text-sm font-semibold text-slate-700">{rubrics.filter((rubric) => rubricIds.includes(rubric.id)).map((rubric) => rubric.name).join(' · ') || 'None selected'}</p><p className="mt-1 text-xs text-slate-500">{rubrics.filter((rubric) => rubricIds.includes(rubric.id)).flatMap((rubric) => rubric.dimensions.map((dimension) => dimension.label)).join(' · ')}</p></div><div className="flex flex-col-reverse gap-3 sm:flex-row"><button type="button" onClick={() => navigate('/')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</button><button type="submit" disabled={submitting} className="academic-primary-button disabled:opacity-50">{submitting ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} {scheduledAt ? 'Schedule GD session' : 'Save draft'}</button></div></section>
          </form>
        </>
      )}
    </div>
