      ...
      {!cohorts.length || !rubrics.length ? (
        ...
        <form className="space-y-5" onSubmit={(event) => handleSubmit(event, Boolean(scheduledAt))}>
          ...
          <section className="ui-card space-y-5 p-6">
            ...
            {!students.length && <div className="grid gap-3 rounded-xl border border-dashed border-indigo-200 bg-indigo-50/50 p-4 md:grid-cols-[.7fr_1fr_1fr_auto]"><input required placeholder="Student ID" value={studentForm.studentId} onChange={(event) => setStudentForm({ ...studentForm, studentId: event.target.value })} className="ui-input" /><input required placeholder="Student name" value={studentForm.name} onChange={(event) => setStudentForm({ ...studentForm, name: event.target.value })} className="ui-input" /><input type="email" placeholder="Email (optional)" value={studentForm.email} onChange={(event) => setStudentForm({ ...studentForm, email: event.target.value })} className="ui-input" /><button type="button" disabled={setupSaving || !studentForm.studentId.trim() || !studentForm.name.trim()} onClick={addStudent} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white disabled:opacity-50"><Plus size={15} /> Add student</button></div>}
          </section>
